...
def query():
    return {
        'query': 'select max(timestamp) as last_activity_time, count(*) as total_events, source_hostname as host from aggregation_table  where source_hostname is not null and type = :type group by source_hostname',
...
def render(results):
    ...
    return  {"series":list(finalResult),"columns":columns,"column":"source_hostname","label":"Server IP","uniquekey":['datacenter'],"columnmap":["source_hostname"]}
