...
def query():
    return {
        'query': 'SELECT count(*) as count, applicationname from detection where criticality = :criticality group by applicationname',
...
def render(result):
    ...
    return {"series":[{'data':data}], 'categories': categories,"column":"applicationname","label":"Applicationname"}
