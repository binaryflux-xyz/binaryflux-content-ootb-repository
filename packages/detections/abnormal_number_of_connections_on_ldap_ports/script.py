...
def algorithm(event):
    stats_label = "ldap_port_detection"
    if event.get("destination_port") in [389, 636]:
        connection_count = stats.count(stats_label)
        if connection_count > 5:  # Threshold for abnormal number of connections
            return 0.85  # Likely LDAP enumeration
    return 0.0
...
def context(event):
    ...
    return (
        ("The occurrence of a large number of connections on LDAP ports"
        ...
        + " Connection Count: " + str(stats.getcount("ldap_port_detection")))
    )
