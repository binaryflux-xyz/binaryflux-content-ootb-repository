...
def algorithm(event):
    if (event.get('action_id', '').strip() in ['APRL', 'DPRL'] and
        'ALTER SERVER ROLE' in event.get('statement', '')):
        return 0.75
    return 0.0

def context(event):
    return 'Server role was modified by executing the query ' + str(event.get('statement', '')) + ' by user ' + str(event.get('server_principal_name', '')) + ' on database ' + str(event.get('database_name', ''))
