...
def algorithm(event):
    if event.get('action_id').strip() == 'PWR':
        return 0.75
    return 0.0


def context(event):
    return 'Server permissions were modified by executing the query ' + str(event.get('statement', '')) + ' by user ' + str(event.get('server_principal_name', '')) + ' on database ' + str(event.get('database_name', ''))
...
def technique():
    return 'Account Manipulation (T1098.001)'
