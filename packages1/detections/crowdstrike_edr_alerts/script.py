...
def investigate():
    # return "email_test_rca"
    return "crowdstrike_alerts_monitoring"
...
def timestamp(event):
    try:
        ...
        epoch_sec = calendar.timegm(dt.timetuple())
...
def algorithm(event):
    try:
        ...
        return 0.50
def get_primary_artifact(event):

    if event.get("ioc_description"):
        return event.get("ioc_description")

    assoc = event.get("associated_files") or []
    if assoc and isinstance(assoc[0], dict):
        return assoc[0].get("filepath")

    quarantine = event.get("quarantined_files") or []
    if quarantine and isinstance(quarantine[0], dict):
        return quarantine[0].get("filename")

    written = event.get("files_written") or []
    if written and isinstance(written[0], dict):
        return "%s\\%s" % (
            written[0].get("filepath", ""),
            written[0].get("filename", "")
        )

    return event.get("filepath")

def get_best_filepath(event):

    assoc = event.get("associated_files") or []
    if assoc and isinstance(assoc[0], dict):
        path = assoc[0].get("filepath")
        if path:
            return path

    quarantine = event.get("quarantined_files") or []
    if quarantine and isinstance(quarantine[0], dict):
        path = quarantine[0].get("filename")
        if path:
            return path
...
def context(event):
    try:
        ...
        filepath = get_best_filepath(event)
        ...
        if process:
            ...
            parts.append("involving process %s" % process)
...
def artifacts():
    try:
        ...
        add("created_time", event.get("created_timestamp"))
        add("user_id", event.get("user_id"))
        add("user_name", event.get("user_name"))
        ...
        add("file_path", get_best_filepath(event))
        ...
        for n in net:
            ...
            if not found_dst:
                ext_ip = device.get("external_ip")
                if ext_ip and "." in str(ext_ip):
                    add("destination_ip", ext_ip)
        ...
        add("composite_id", event.get("composite_id"))
...
def correlationArtifacts():
    return ["host","file_hash","file_name","file_path","composite_id","source_ip", "destination_ip"]
