...
def init(event):
    ...
    if mail_folder == "sent":
      print("at start of init sending unusual recipients")
      recipients = []
      ...
      return "mail folder is not type of sent"
...
def context(event_data):
    ...
    if len(recipients_with_anomaly) > 0:
        ...
        msg = "An unfamiliar email address was detected while sending an email from " + event_data['email_from_address'] + " to " + str(to_value) + " with subject " + event_data['email_subject']
...
def artifacts():
    return stats.collect(['email_to_address', 'email_cc_address', 'email_bcc_address', 'email_reply_to_address','source_location'])
